...
import { ApiInfluenceCell, Piece as ApiPiece, Origin } from "./types";
...
function App() {
  ...
  const [draggedSourceSquare, setDraggedSourceSquare] = useState<Square | null>(
    null
  );
  ...
  const [resetPieceSelection, setResetPieceSelection] =
    useState<boolean>(false);
  const [influenceBoard, setInfluenceBoard] = useState<
    (ApiInfluenceCell | null)[][]
  >(
    Array(8)
      .fill(null)
  ...
  const [selectedBoardPiece, setSelectedBoardPiece] = useState<Square | null>(
    null
  );
  const [highlightedOrigins, setHighlightedOrigins] = useState<Origin[] | null>(
    null
  );
  ...
  const getPiecesFromFen = (
    fen: string,
    currentView: "allies" | "enemies"
  ): ApiPiece[] => {
    ...
    for (let row = 0; row < 8; row++) {
      // Iterate rows first (0-7 corresponds to 8-1)
      for (let col = 0; col < 8; col++) {
        // Iterate columns (0-7 corresponds to a-h)
  ...
  const onDrop = (sourceSquare: Square, targetSquare: Square) => {
    ...
    if (draggedPiece) {
      ...
      gameCopy.put(
        { type: draggedPiece.type, color: draggedPiece.team },
        targetSquare
      );
  ...
  const highlightInfluencedSquares = (square: Square) => {
    ...
    for (let i = 0; i < 8; i++) {
      // i = row index in influenceBoard (0 = rank 1)
      for (let j = 0; j < 8; j++) {
        // j = col index in influenceBoard (0 = col 'a')
        const cell = influenceBoard[i][j];
        if (cell?.origins) {
          const pieceIsOrigin = cell.origins.some(
            (origin) => origin.x === pieceX && origin.y === pieceY
          );
          if (pieceIsOrigin) {
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    const targetSquare = `${String.fromCharCode(97 + file)}${
      rank + 1
    }` as Square;
  ...
  return (
    <div className="app">
      ...
      <p className="app-subtitle">
        Apprenez et visualisez les zones d'influence aux échecs.
      </p>
      ...
      <div className="main-content">
          ...
          <div className="legend-container">
            ...
            <div className="legend-item">
              ...
              <div className="legend-text">
                Cases contrôlées par les pièces du camp sélectionné
              </div>
            </div>
            <div className="legend-item">
              <div className="legend-color green"></div>
              <div className="legend-text">
                Pièces du camp sélectionné défendues
              </div>
            </div>
            <div className="legend-item">
            ...
            <div className="legend-item">
              ...
              <div className="legend-text">
                Nombre de pièces du camp sélectionné ciblant une case
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="board-section">
          ...
          <div className="controls">
            ...
            <button
              onClick={() => setShowInfluenceColors(!showInfluenceColors)}
              className="control-button"
            >
              <span className="button-icon">
                {showInfluenceColors ? "🙈" : "👁️"}
              </span>
              <span className="button-text">
                {showInfluenceColors ? "Masquer" : "Afficher"}
              </span>
            </button>
          </div>
        </div>
      ...
      <div className="about-section">
        ...
        >
          ...
          <span className="about-toggle">{showAboutSection ? "▼" : "▶"}</span>
        </div>
        {showAboutSection && (
          <div className="about-content">
            <p>
              King's Board est un outil pédagogique pour apprendre les zones
              d'influence aux échecs.
            </p>
            <p>
              Placez les pièces sur l'échiquier pour visualiser instantanément
              leur influence sur le plateau.
            </p>
            <p>Le code couleur indique :</p>
            <ul className="instructions-list">
              <li>
                Jaune : Cases contrôlées par les pièces du camp sélectionné
              </li>
              <li>Vert : Pièces du camp sélectionné défendues</li>
              <li>Rouge : Pièces adverses en ligne de vue</li>
            </ul>
            <p>
              Utilisez les boutons pour changer de vue (alliés/ennemis) et
              l'orientation du plateau.
            </p>
            <p>
              Idéal pour les débutants comme pour les joueurs expérimentés
              souhaitant approfondir leur compréhension stratégique.
            </p>
          </div>
        )}
      </div>
